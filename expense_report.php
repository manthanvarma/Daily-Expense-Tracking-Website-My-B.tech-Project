...
if(isset($_POST['category_name']) && isset($_POST['submit'])) {
	...
	$sql = "SELECT * from user u JOIN expense e ON u.id = e.id JOIN category c ON e.category_id = c.category_id WHERE u.id = :id AND e.category_id = :category_name AND e.expense_date BETWEEN :startdate AND :enddate ORDER BY expense_date ASC";
	$data = array(":id"=>$id, ":category_name"=>$category_name, ":startdate"=>$startdate, ":enddate"=>$enddate);
...
if(isset($_POST['category_name']) && $_POST['category_name'] == 'none' && isset($_POST['submit'])) {
		$startdate = $_POST['startdate'];
    $enddate = $_POST['enddate'];
	$startdate = date("Y-m-d", strtotime($startdate));
	$enddate = date("Y-m-d", strtotime($enddate));
	...
	$sql = "SELECT * from user u JOIN expense e ON u.id = e.id JOIN category c ON e.category_id = c.category_id WHERE u.id = :id AND e.expense_date BETWEEN :startdate AND :enddate ORDER BY expense_date ASC";
...
<body>
	...
	// let timerInterval
      // Swal.fire({
        // title: 'Your Expense Report is Being Generated!',
		// icon: 'success',
              ...
              <div class="card-body">
                <div class="container-fluid d-flex justify-content-between">
                  <div class="col-lg-3 ps-0">
                    <a href="#" class="noble-ui-logo d-block mt-3">Expense<span>Report</span></a>                 
                  ...
                  <div class="table-responsive w-100">
                      <table class="table table-bordered">
						...
						foreach($result as $r) {
							...
							$expense_date1 = date("d-M-Y", strtotime($expense_date));
...
<?php }}else {echo  '<script>location.replace("expensereport.php?mode=notfound")</script>'; } ?>
								<tbody>
								<td class = "text-center" colspan = "3"><b></b></td>
								<td class = "text-center bg-light"><b>Total Expense</b></td>
