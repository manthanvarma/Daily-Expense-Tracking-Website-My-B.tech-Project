...
extract($_POST);
$sql = "SELECT * FROM income i JOIN incomecategory c ON i.i_category_id = c.i_category_id AND i.id = $id";
...
if ($total_pages > 1) {
    $pagination = paginate ('manage_income.php?' . $url_string, $cur_page, $total_pages);
    $url_string .= "&page=" . $cur_page;
}

$sql ="SELECT * FROM income i JOIN incomecategory c ON i.i_category_id = c.i_category_id AND i.id = :id limit $page_start , $per_page  ";
...
<html>
	...
	<div class = "page-wrapper">
		...
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" aria-current="page">Manage Income</li>
		</ol>
		</nav>
		<div class="row">
            ...
            <div class="card">
			  ...
			  <?php if (isset($_GET['mode']) && $_GET['mode'] == 'inserted') { ?>
					...
					<?php } ?>
								<h6 class="card-title">Manage Income</h6>
										...
										<table class="table">
											...
											<tbody>
												<tr>
													...
													<td><a href="edit_income.php?income_id=<?php echo "$income_id"; ?>"><div class="btn btn-outline-primary"><i class="ms-2  me-2 icon-md" data-feather="edit"></i></div></a></td>
													<td><a href="delete_income.php?income_id=<?php echo "$income_id"; ?>" onclick="return confirm('Are you sure?')"><div class="btn btn-outline-Danger"><i class="ms-2  me-2 icon-md" data-feather="trash"></i></div></a></td>
												</tr>
												<?php }} $conn = null;?>
											</tbody>
										</table>
